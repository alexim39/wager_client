...
export interface WithdrawDetailsInterface {
    userId: string;
    bankName: string;
    accountNo: string;
    amount: number;
...
@Injectable({
  ...
  export class WithdrawDetailsService {
    ...
    // withdraw
    withdraw(withdrawDetails: WithdrawDetailsInterface): Observable<ServerResponse> {
      return this.http.post<ServerResponse>(`${this.API_DOMAIN}/api/withdraw`, withdrawDetails, httpOptions)
        .pipe(
